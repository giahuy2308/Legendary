...
def import_vcnv(vcnv_ws,exam):
    step = 2 
    get_question(VCNVQuestion, exam, vcnv_ws, step, 1, 0, question_type="CNV", picture_opened=True, question_shown=False)
    step = 4
    get_question(VCNVQuestion, exam, vcnv_ws, step, 4, 0, question_type="HN", picture_opened=False, question_shown=False)
    step = 8
    get_question(VCNVQuestion, exam, vcnv_ws, step, 1, 0, question_type="TT", picture_opened=False, question_shown=False)

def import_tt(tt_ws, exam):
    step = 3
    get_question(TTQuestion, exam, tt_ws, step, 4, 0)

def import_vd(vd_ws, exam):
    for player_no in range(1,5):
        step = 4 + (player_no-1) * 8
        get_question(VDQuestion, exam, vd_ws, step, 3, player_no, question_value=20)
        get_question(VDQuestion, exam, vd_ws, step+3, 3, player_no, question_value=30)
...
def import_exam(exam_dir, file_name):
    global DIR 
    DIR = os.path.join(os.path.join(settings.MEDIA_ROOT, "Exam", exam_dir))
